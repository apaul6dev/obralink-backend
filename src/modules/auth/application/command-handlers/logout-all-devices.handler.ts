import { Inject, Logger } from '@nestjs/common';
import { CommandHandler, ICommandHandler } from '@nestjs/cqrs';
import { randomUUID } from 'crypto';
import { AuthEvent } from '../../domain/entities/auth-event.entity';
import { AuthEventType } from '../../domain/enums/auth-event-type.enum';
import { AuthEventRepository } from '../../domain/repositories/auth-event.repository.interface';
import { RefreshTokenRepository } from '../../domain/repositories/refresh-token.repository.interface';
import { AUTH_EVENT_REPOSITORY, REFRESH_TOKEN_REPOSITORY, USER_SESSION_REPOSITORY } from '../../domain/repositories/repository-tokens';
import { UserSessionRepository } from '../../domain/repositories/user-session.repository.interface';
import { LogoutAllDevicesCommand } from '../commands/logout-all-devices.command';

@CommandHandler(LogoutAllDevicesCommand)
export class LogoutAllDevicesHandler implements ICommandHandler<LogoutAllDevicesCommand> {
  private readonly logger = new Logger(LogoutAllDevicesHandler.name);

  constructor(
    @Inject(USER_SESSION_REPOSITORY) private readonly sessions: UserSessionRepository,
    @Inject(REFRESH_TOKEN_REPOSITORY) private readonly refreshTokens: RefreshTokenRepository,
    @Inject(AUTH_EVENT_REPOSITORY) private readonly authEvents: AuthEventRepository,
  ) {}

  async execute(command: LogoutAllDevicesCommand): Promise<{ success: true }> {
    const now = new Date();
    await this.refreshTokens.revokeActiveByUserId(command.userId, now);
    await this.sessions.revokeAllActiveByUserId(command.userId, now);
    await this.authEvents.save(new AuthEvent(randomUUID(), AuthEventType.LOGOUT_ALL_DEVICES, command.userId, command.tenantId, command.sessionId, null, command.ipAddress, command.userAgent, null, now));
    this.logger.log(`Logout all devices succeeded userId=${command.userId} tenantId=${command.tenantId ?? 'global'} triggeringSessionId=${command.sessionId}`);
    return { success: true };
  }
}
